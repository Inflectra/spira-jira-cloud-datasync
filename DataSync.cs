...
using Inflectra.SpiraTest.PlugIns.JiraCloudDataSync.SpiraSoapService;
using System.ServiceModel;
using System.Net.Security;
using Inflectra.SpiraTest.PlugIns.JiraCloudDataSync.JiraClient;

namespace Inflectra.SpiraTest.PlugIns.JiraCloudDataSync
{
    /// <summary>
    /// Contains all the logic necessary to sync SpiraTest 5.x with Jira Cloud
    /// </summary>
    public class DataSync : IDataSyncPlugIn
    {
                    ...
                    if (customPropertyDataMapping != null)
                    {
                        ...
                        else if (customProperty.CustomPropertyTypeId == (int)Constants.CustomPropertyType.User)
                        {
                            ...
                            foreach (JiraCustomFieldValue jiraCustomFieldValue in jiraCustomFieldValues)
                            {
                                ...
                                {
                                    //We need to get the Spira user id from the equivalent Jira account id
                                    string jiraUserLogin = jiraCustomFieldValue.Value.StringValue;
                                    if (!String.IsNullOrEmpty(jiraUserLogin))
                                    {
                                        UserMappingEntry userMappingEntry = FindUserMappingByJiraAccountId(jiraCustomFieldValue.Value.StringValue, userMappings, spiraSoapService, jiraManager);
                                        if (userMappingEntry != null)
                                        {
                                            InternalFunctions.SetCustomPropertyValue(remoteArtifact, customProperty.PropertyNumber, userMappingEntry.SpiraUserId);
                                        }
                                        else
                                        {
                                            LogErrorEvent("Unable to find a matching " + productName + " user that matches JIRA user with accountId=" + jiraCustomFieldValue.Value.StringValue + " so leaving property null.", EventLogEntryType.Warning);
                                        }
                                    }
                                }
                            }
        ...
        private void ProcessJiraIssueAsIncident(int projectId, SoapServiceClient spiraImportExport, JiraIssue jiraIssue, List<RemoteDataMapping> newIncidentMappings, List<RemoteDataMapping> newReleaseMappings, List<RemoteDataMapping> oldReleaseMappings, Dictionary<int, RemoteDataMapping> customPropertyMappingList, Dictionary<int, RemoteDataMapping[]> customPropertyValueMappingList, RemoteCustomProperty[] incidentCustomProperties, RemoteDataMapping[] incidentMappings, string jiraProject, JiraManager jiraManager, string productName, RemoteDataMapping[] severityMappings, RemoteDataMapping[] priorityMappings, RemoteDataMapping[] statusMappings, RemoteDataMapping[] typeMappings, RemoteDataMapping[] userMappings, RemoteDataMapping[] releaseMappings, RemoteDataMapping[] incidentComponentMappings)
        {
            ...
            if (incidentMapping == null)
            {
                ...
                if (!this.onlyCreateNewItemsInJira)
                {
                    ...
                    if (jiraIssue.Fields.Reporter != null && !String.IsNullOrEmpty(jiraIssue.Fields.Reporter.AccountId))
                    {
                        UserMappingEntry dataMapping = FindUserMappingByJiraAccountId(jiraIssue.Fields.Reporter.AccountId, userMappings, spiraImportExport, jiraManager);
                        if (dataMapping == null)
                        {
                            //We can't find the matching user so log and ignore
                            LogErrorEvent("Unable to locate mapping entry for Jira user " + jiraIssue.Fields.Reporter.AccountId + " so using synchronization user as detector.", EventLogEntryType.Warning);
                        }
                        else
                        {
                            remoteIncident.OpenerId = dataMapping.SpiraUserId;
            ...
            try
            {
                ...
                if (remoteIncident != null)
                {
                    ...
                    if (jiraComments != null && jiraComments.Count > 0)
                    {
                        foreach(JiraComment jiraComment in jiraComments)
                        {
                            ...
                            if (!alreadyAdded)
                            {
                                ...
                                LogTraceEvent(eventLog, "Looking for comments author: '" + jiraComment.Author.AccountId + "'\n", EventLogEntryType.Information);
                                int? creatorId = null;
                                UserMappingEntry dataMapping = FindUserMappingByJiraAccountId(jiraComment.Author.AccountId, userMappings, spiraImportExport, jiraManager);
                                if (dataMapping == null)
                                {
                                    LogTraceEvent(eventLog, "Looking for comments update-author: '" + jiraComment.UpdateAuthor.AccountId + "'\n", EventLogEntryType.Information);
                                    dataMapping = FindUserMappingByJiraAccountId(jiraComment.UpdateAuthor.AccountId, userMappings, spiraImportExport, jiraManager);
                                    if (dataMapping != null)
                                    {
                                        creatorId = dataMapping.SpiraUserId;
                                    }
                                }
                                else
                                {
                                ...
                                {
                                    LogTraceEvent(eventLog, "Got the comment creator: " + creatorId.ToString() + "\n", EventLogEntryType.Information);
                                }
                    ...
                    if (jiraIssue.Fields.Assignee == null || String.IsNullOrEmpty(jiraIssue.Fields.Assignee.AccountId))
                    {
                        remoteIncident.OwnerId = null;
                    }
                    else
                    {
                        UserMappingEntry dataMapping = FindUserMappingByJiraAccountId(jiraIssue.Fields.Assignee.AccountId, userMappings, spiraImportExport, jiraManager);
                        if (dataMapping == null)
                        {
                            //We can't find the matching user so log and ignore
                            LogErrorEvent("Unable to locate mapping entry for Jira user " + jiraIssue.Fields.Assignee.AccountId + " so ignoring the assignee change", EventLogEntryType.Warning);
                        }
                        else
                        {
                            remoteIncident.OwnerId = dataMapping.SpiraUserId;
            ...
            if (requirementMapping == null)
            {
                ...
                if (!this.onlyCreateNewItemsInJira && InternalFunctions.FindMappingByExternalKey(projectId, jiraIssue.Key.ToString(), newRequirementMappings, false) == null)
                {
                    ...
                    if (jiraIssue.Fields.Reporter != null && !String.IsNullOrEmpty(jiraIssue.Fields.Reporter.AccountId))
                    {
                        ...
                        {
                            remoteRequirement.AuthorId = dataMapping.SpiraUserId;
            ...
            try
            {
                ...
                if (remoteRequirement != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            remoteRequirement.OwnerId = dataMapping.SpiraUserId;
        ...
        private void ProcessIncident(int projectId, SoapServiceClient spiraImportExport, RemoteIncident remoteIncident, List<RemoteDataMapping> newIncidentMappings, List<RemoteDataMapping> newReleaseMappings, List<RemoteDataMapping> oldReleaseMappings, Dictionary<int, RemoteDataMapping> customPropertyMappingList, Dictionary<int, RemoteDataMapping[]> customPropertyValueMappingList, RemoteCustomProperty[] incidentCustomProperties, RemoteDataMapping[] incidentMappings, JiraProject jiraProject, JiraManager jiraManager, string productName, RemoteDataMapping[] severityMappings, RemoteDataMapping[] priorityMappings, RemoteDataMapping[] statusMappings, RemoteDataMapping[] typeMappings, RemoteDataMapping[] userMappings, RemoteDataMapping[] releaseMappings, RemoteDataMapping[] incidentComponentMappings)
        {
            ...
            if (InternalFunctions.FindMappingByInternalId(projectId, incidentId, incidentMappings) == null)
            {
                ...
                UserMappingEntry userMappingEntry = FindUserMappingBySpiraId(remoteIncident.OpenerId.Value, userMappings, spiraImportExport, jiraManager);
                if (userMappingEntry == null)
                {
                    //We can't find the matching user so just use the current user
                    ...
                    userMappingEntry = FindUserMappingByJiraEmailAddress(externalLogin, userMappings, spiraImportExport, jiraManager);
                    if (userMappingEntry == null || userMappingEntry.JiraAccountId == null)
                    {
                        //Final fallback is to use the current accountId
                        JiraUser currentUser = jiraManager.GetCurrentUser();
                        if (currentUser == null)
                        {
                            LogErrorEvent("Unable to locate mapping entry for Jira user '" + externalLogin + "', and unable to get sync user so leaving the reporter unset", EventLogEntryType.Warning);
                        }
                        else
                        {
                            jiraIssue.Fields.Reporter = currentUser;
                        }
                    }
                    else
                    {
                        jiraIssue.Fields.Reporter = new JiraUser(userMappingEntry.JiraAccountId);
                    }
                }
                else
                {
                    jiraIssue.Fields.Reporter = new JiraUser(userMappingEntry.JiraAccountId);
                    LogTraceEvent(eventLog, "Set issue reporter to '" + userMappingEntry.JiraAccountId + "'\n", EventLogEntryType.Information);
                }

                //Now set the assignee - set NULL if no assignee (owner) in SpiraTest
                if (remoteIncident.OwnerId.HasValue)
                {
                    userMappingEntry = FindUserMappingBySpiraId(remoteIncident.OwnerId.Value, userMappings, spiraImportExport, jiraManager);
                    if (userMappingEntry == null || userMappingEntry.JiraAccountId == null)
                    {
                        //We can't find the matching user so just leave unset
                        LogErrorEvent("Unable to locate mapping entry for owner user id " + remoteIncident.OwnerId.Value + " so leaving unset", EventLogEntryType.Warning);
                    }
                    else
                    {
                        jiraIssue.Fields.Assignee = new JiraUser(userMappingEntry.JiraAccountId);
                            ...
                            else if (customProperty.CustomPropertyTypeId == (int)Constants.CustomPropertyType.User)
                            {
                                ...
                                if (artifactCustomProperty.IntegerValue.HasValue)
                                {
                                    ...
                                    if (customPropertyDataMapping != null && !String.IsNullOrEmpty(customPropertyDataMapping.ExternalKey))
                                    {
                                        ...
                                        userMappingEntry = FindUserMappingBySpiraId(artifactCustomProperty.IntegerValue.Value, userMappings, spiraImportExport, jiraManager);
                                        if (userMappingEntry != null)
                                        {
                                            int customFieldId;
                                            if (Int32.TryParse(externalCustomField, out customFieldId))
                                            {
                                                ...
                                                cpv.StringValue = userMappingEntry.JiraAccountId;
                                                customPropertyValues.Add(customFieldId, cpv);
                                                LogTraceEvent(eventLog, "Added user custom property field value: " + customProperty.Name + " (Value=" + userMappingEntry.JiraAccountId + ")\n", EventLogEntryType.Information);
                                            }
                                            else
                                            {
                                                LogErrorEvent("Unable to set a value on JIRA custom field '" + externalCustomField + "' because the custom field id is not an integer.", EventLogEntryType.Warning);
        ...
        private void ProcessUpdatedIncident(int projectId, SoapServiceClient spiraImportExport, RemoteIncident remoteIncident, List<RemoteDataMapping> newReleaseMappings, Dictionary<int, RemoteDataMapping> customPropertyMappingList, Dictionary<int, RemoteDataMapping[]> customPropertyValueMappingList, RemoteCustomProperty[] incidentCustomProperties, RemoteDataMapping[] incidentMappings, JiraProject jiraProject, JiraManager jiraManager, string productName, RemoteDataMapping[] severityMappings, RemoteDataMapping[] priorityMappings, RemoteDataMapping[] statusMappings, RemoteDataMapping[] typeMappings, RemoteDataMapping[] userMappings, RemoteDataMapping[] releaseMappings, RemoteDataMapping[] incidentComponentMappings, List<string> recentlyUpdatedFromJira)
        {
            ...
            if (incidentMapping != null && !recentlyUpdatedFromJira.Contains(incidentMapping.ExternalKey))
            {
                ...
                if (jiraIssue != null)
                {
                    ...
                    if (userMappingEntry == null)
                    {
                        //We can't find the matching user so don't change
                        LogErrorEvent("Unable to locate mapping entry for opener user id " + remoteIncident.OpenerId + " so not changing value", EventLogEntryType.Warning);
                    }
                    else
                    {
                        jiraIssue.Fields.Reporter = new JiraUser(userMappingEntry.JiraAccountId);
                    ...
                    if (remoteIncident.OwnerId.HasValue)
                    {
                        ...
                        {
                            //We can't find the matching user so don't change
                            LogErrorEvent("Unable to locate mapping entry for owner user id " + remoteIncident.OwnerId.Value + " so not changing value", EventLogEntryType.Warning);
                        }
                        else
                        {
                            jiraIssue.Fields.Assignee = new JiraUser(userMappingEntry.JiraAccountId);
                    ...
                    //Next see if we have any comments to add to JIRA
                    //We need to make sure we don't add duplicate ones, by checking for string match and special tokens
                    RemoteComment[] remoteComments = spiraImportExport.Incident_RetrieveComments(incidentId);
                    if (remoteComments != null && remoteComments.Length > 0)
                    {
                        try
        ...
        public ServiceReturnType Execute(Nullable<DateTime> lastSyncDate, DateTime serverDateTime)
        {
            ...
            try
            {
                ...
                foreach (SpiraSoapService.RemoteDataMapping projectMapping in projectMappings)
                {
                    ...
                    //Get the create incident meta-data for this project (so that we can verify which fields we need to provide)
                    //We do it per project to better support 'next-gen' style Jira projects
        ...
        /// <summary>
        /// Finds a user mapping entry from the Spira id
        /// </summary>
        /// <param name="internalId">The internal id (Spira)</param>
        /// <param name="dataMappings">The list of mappings</param>
        /// <returns>The matching entry or Null if none found</returns>
        protected UserMappingEntry FindUserMappingBySpiraId(int internalId, RemoteDataMapping[] dataMappings, SoapServiceClient client, JiraManager jiraManager)
        {
            //See if we have the auto-map users option enabled
            //This assumes that they have the same email addresses in both systems
            if (this.autoMapUsers)
            {
                ...
                //Now get the matching user in Jira
                try
                {
                    List<JiraUser> jiraUsers = jiraManager.FindUsersByEmailAddress(remoteUser.EmailAddress);
                    if (jiraUsers.Count > 0)
                    {
                        UserMappingEntry userMapping = new UserMappingEntry();
                        userMapping.SpiraUserId = remoteUser.UserId.Value;
                        userMapping.JiraAccountId = jiraUsers[0].AccountId;
                        userMapping.JiraEmailAddress = jiraUsers[0].EmailAddress;
                        return userMapping;
                    }
                    else
                    {
                        LogErrorEvent("Unable to find Jira user with email address: '" + remoteUser.EmailAddress + "'", EventLogEntryType.Warning);
                        return null;
                    }
                }
                catch (Exception exception)
                {
                    //Jira sometimes throws back a 404 if the user cannot be found
                    LogErrorEvent("Unable to find Jira user with email address: '" + remoteUser.EmailAddress + "' - " + exception.Message, EventLogEntryType.Warning);
                    return null;
                }
            }
            else
            {
                RemoteDataMapping dataMapping = InternalFunctions.FindMappingByInternalId(internalId, dataMappings);
                if (dataMapping == null)
                {
                    return null;
                }
                ...
                try
                {
                    //We might be mapped to either an account ID or an email address, so try both
                    List<JiraUser> jiraUsers = jiraManager.FindUsersByEmailAddress(dataMapping.ExternalKey);
                    if (jiraUsers.Count > 0)
                    {
                        UserMappingEntry userMapping = new UserMappingEntry();
                        userMapping.SpiraUserId = internalId;
                        userMapping.JiraAccountId = jiraUsers[0].AccountId;
                        userMapping.JiraEmailAddress = jiraUsers[0].EmailAddress;
                        return userMapping;
                    }
                    ...
                    {
                        JiraUser jiraUser = jiraManager.GetUser(dataMapping.ExternalKey);
                        if (jiraUser != null)
                        {
                            UserMappingEntry userMapping = new UserMappingEntry();
                            userMapping.SpiraUserId = internalId;
                            userMapping.JiraAccountId = jiraUser.AccountId;
                            return userMapping;
                        }
                        else
                        {
                            LogErrorEvent("Unable to find Jira user with email address or accountId: '" + dataMapping.ExternalKey + "'", EventLogEntryType.Warning);
                            return null;
                        }
                    }
                }
                ...
                {
                    //Jira sometimes throws back a 404 if the user cannot be found
                    LogErrorEvent("Unable to find Jira user with email address or accountId: '" + dataMapping.ExternalKey + "' - " + exception.Message, EventLogEntryType.Warning);
                    return null;
                }
            }
        }
        ...
        /// <summary>
        /// Finds a user mapping entry from the Jira email address
        /// </summary>
        /// <param name="emailAddress">The Jira email address</param>
        /// <param name="dataMappings">The list of mappings</param>
        /// <returns>The matching entry or Null if none found</returns>
        /// <remarks>
        /// The Jira accountId is not set in this case
        /// </remarks>
        protected UserMappingEntry FindUserMappingByJiraEmailAddress(string emailAddress, RemoteDataMapping[] dataMappings, SoapServiceClient client, JiraManager jiraManager)
        {
            //See if we have the auto-map users option enabled
            //This assumes that they have the same usernames / email addresses in both systems
            if (this.autoMapUsers)
            {
                try
                {
                    RemoteUser remoteUser = client.User_RetrieveByUserName(emailAddress, true);
                    if (remoteUser == null)
                    {
                        return null;
                    }
                    UserMappingEntry userMapping = new UserMappingEntry();
                    userMapping.SpiraUserId = remoteUser.UserId.Value;
                    userMapping.JiraEmailAddress = emailAddress;
                    return userMapping;
                }
                catch (Exception)
                {
            ...
            {
                RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(emailAddress, dataMappings);
                if (dataMapping == null)
                {
                    return null;
                }
                UserMappingEntry userMapping = new UserMappingEntry();
                userMapping.SpiraUserId = dataMapping.InternalId;
                userMapping.JiraEmailAddress = emailAddress;
                return userMapping;
            }
        }
        ...
        /// <summary>
        /// Finds a user mapping entry from the Jira account Id
        /// </summary>
        /// <param name="jiraAccountId">The Jira account ID Guid</param>
        /// <param name="dataMappings">The list of mappings</param>
        /// <returns>The matching entry or Null if none found</returns>
        protected UserMappingEntry FindUserMappingByJiraAccountId(string jiraAccountId, RemoteDataMapping[] dataMappings, SoapServiceClient client, JiraManager jiraManager)
        {
            //First get the specified user in Jira
            try
            {
                JiraUser jiraUser = jiraManager.GetUser(jiraAccountId);
                if (jiraUser == null)
                {
                    return null;
                }
                ...
                if (this.autoMapUsers)
                {
                    try
                    {
                        RemoteUser remoteUser = client.User_RetrieveByUserName(jiraUser.EmailAddress, true);
                        if (remoteUser == null)
                        {
                            return null;
                        }
                        ...
                        userMapping.SpiraUserId = remoteUser.UserId.Value;
                        userMapping.JiraAccountId = jiraUser.AccountId;
                        userMapping.JiraEmailAddress = jiraUser.EmailAddress;
                        return userMapping;
                    }
                    catch (Exception)
                    {
                ...
                {
                    //We first try mapping on account ID and if that does not work, we map on email address
                    RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(jiraAccountId, dataMappings);
                    if (dataMapping == null)
                    {
                        dataMapping = InternalFunctions.FindMappingByExternalKey(jiraUser.EmailAddress, dataMappings);
                    }
                    if (dataMapping == null)
                    {
                        return null;
            ...
            {
                //Jira sometimes throws back a 404 if the user cannot be found
                LogErrorEvent("Unable to find Jira user with accountId: '" + jiraAccountId + "' - " + exception.Message, EventLogEntryType.Warning);
                return null;
            }
        }
